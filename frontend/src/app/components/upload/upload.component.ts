...
const INDIAN_BANKS = [
  'AU Small Finance Bank','HDFC Bank','SBI – State Bank of India',
  'ICICI Bank','Axis Bank','Kotak Mahindra Bank','Punjab National Bank',
...
@Component({
  ...
  template: `
    <div class="page" style="max-width:820px">
      ...
      <p class="page-subtitle">Upload a bank statement PDF and fill in the analyst details</p>

      <!-- Step: Form -->
      <ng-container *ngIf="step === 'form'">
        <!-- Drop zone -->
        <div class="drop-zone card" [class.dragover]="dragging"
          (dragover)="$event.preventDefault(); dragging=true"
          (dragleave)="dragging=false"
          (drop)="onDrop($event)"
          (click)="fileInput.click()">
          <input #fileInput type="file" accept=".pdf" (change)="onFile($event)">
          <div class="drop-icon">📂</div>
          <div class="drop-text">
            <strong>Click to browse</strong> or drag & drop your PDF
          </div>
          <div class="drop-text text-sm mt-8" style="margin-top:6px">Max 50MB · PDF only</div>
          <div class="drop-filename" *ngIf="file">{{ file.name }}</div>
        </div>
        ...
        <div class="card mt-16">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input class="form-input" [(ngModel)]="meta.accountNumber" placeholder="Last 4 digits or full">
            </div>
            <div class="form-group">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input class="form-input" [(ngModel)]="meta.notes" placeholder="Optional notes">
            </div>
          </div>
          ...
          <div class="flex-center gap-12" style="margin-top:8px">
            ...
            <button class="btn btn-primary ml-auto" (click)="submit()" [disabled]="!file || !meta.customerName">
              Parse Statement →
            </button>
          </div>
        </div>

        <div class="card mt-16" style="background:var(--amber-dim);border-color:rgba(255,179,71,0.2)" *ngIf="error">
      ...
      <ng-container *ngIf="step === 'uploading'">
        <div class="card" style="text-align:center;padding:64px">
          ...
          <h3 style="font-family:'Inter',sans-serif;font-weight:400;color:var(--text-2);margin-bottom:8px">Parsing statement…</h3>
          <p class="text-muted text-sm">Running multi-engine extraction. This takes 5–15 seconds.</p>
      ...
      <ng-container *ngIf="step === 'password'">
        <div class="card" style="max-width:420px;margin:0 auto;text-align:center;padding:40px">
          ...
          <h2 style="margin-bottom:8px">PDF is Password Protected</h2>
          <p class="text-muted text-sm" style="margin-bottom:24px">
            Enter the password provided by your bank to unlock this statement.
          </p>
          <div class="form-group" style="text-align:left">
            ...
            <div style="color:var(--red);font-size:0.8rem;margin-top:6px" *ngIf="wrongPassword">
              ✕ Incorrect password. Please try again.
            </div>
          </div>
          <div class="flex-center gap-10" style="margin-top:16px">
            <button class="btn btn-ghost" (click)="reset()">Start Over</button>
            <button class="btn btn-primary ml-auto" (click)="submitPassword()" [disabled]="!password || unlocking">
      ...
      <ng-container *ngIf="step === 'done' && result">
        <div class="card" style="text-align:center;padding:48px">
          ...
          <p class="text-muted text-sm" style="margin-bottom:24px">
            {{ result.totalTransactions }} transactions extracted · {{ (result.confidence! * 100).toFixed(0) }}% confidence
          </p>
          <div class="flex-center gap-12" style="justify-content:center">
            <button class="btn btn-ghost" (click)="reset()">Upload Another</button>
            <button class="btn btn-primary" (click)="router.navigate(['/statements', result.id])">
              View Results →
            </button>
          </div>
        </div>
      ...
      <ng-container *ngIf="step === 'error'">
        <div class="card" style="text-align:center;padding:48px;border-color:rgba(255,77,109,0.3)">
          <div style="font-size:2.5rem;margin-bottom:16px">❌</div>
...
export class UploadComponent {
  ...
  onDrop(evt: DragEvent) {
    ...
    else this.error = 'Please drop a PDF file.';
  }
  ...
  submit() {
    ...
    this.svc.upload(this.file, this.meta).subscribe({
      ...
      error: (e) => { this.step = 'error'; this.error = e?.error?.error || 'Upload failed. Is the backend running?'; }
    });
  }
  ...
  handleResult(stmt: Statement) {
    ...
    if (stmt.status === 'PENDING_PASSWORD') {
      this.step = 'password';
    } else if (stmt.status === 'DONE') {
      this.result = stmt;
      this.step = 'done';
    } else {
      this.step = 'error';
      this.error = stmt.errorMessage || 'Extraction failed.';
    }
  }
